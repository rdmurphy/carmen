...
function verifyContext(context, peers, strict, loose, indexes, options, geocoder) {
    ...
    for (let c = 0; c < context.length; c++) {
        ...
        // generally if relevance is tied, it's properties of the main result feature (context[0]) that we care about
        // for tie-breaking, like its score, but if even *those* are tied, make it possible to express a preference
        // based on the next-most-specific context entry
        if (c > 0 && contextRank === null && !options.proximity) {
            contextRank = indexes[context[c].properties['carmen:index']].geocoder_context_tiebreaker_rank;
        }
    ...
    context[0].properties['carmen:context_rank'] = contextRank || 0;
...
function sortContext(a, b) {
    ...
    // if everything is *still* tied, see if we can tiebreak on the most-specific context element
    if (a[0].properties['carmen:context_rank'] > b[0].properties['carmen:context_rank']) return -1;
    if (a[0].properties['carmen:context_rank'] < b[0].properties['carmen:context_rank']) return 1;
